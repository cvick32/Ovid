...
from z3_defs import Write, Read, ConstArr
...
class Interpolator:
    ...
    def get_def_interpolant_sexprs(self):
        ...
        for defn in self.z3_model:
            if defn.decl() not in [Write, Read, ConstArr]:
    ...
    def get_interpolant_asserts(self):
        ...
        for step, exprs_at_step in enumerate(self.exprs):
            ...
            and_sexpr.replace_heads_in_all_body("write_int_int", "store")
            and_sexpr.replace_heads_in_all_body("read_int_int", "select")
    ...
    def interpolate(self):
        ...
        if interpolant_str == "SAT":
            return None
        print("Interpolants:")
        self.all_interpolants = Translate().parse_sexpr_string(interpolant_str)[0].body
